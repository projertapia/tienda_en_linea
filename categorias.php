...
session_start();
	...
	if(isset($_GET['cat']) AND $_GET['cat'] !=""){
		$cat=$_GET['cat'];
		$title=$cat;
		$searchby="AND UPPER(p.categoria_producto) LIKE UPPER('%$cat%')";
	}else{
		$cat = NULL;
		$searchby="";
		$title="Todas";
...
WHERE p.activo_producto = 1 AND p.stock_producto >= 1 AND p.id_tienda_producto = t.id_tienda $searchby $OrderBy";
...
<head>
<meta charset="utf-8">
<title>Categoría <?php echo $title;?> | Ecommerce</title>
...
<body>
	...
	<section id="home-producto" style="background-image: linear-gradient(rgba(255, 255, 255, 0.0) 50%, rgba(255, 255, 255, 1) 100%), url(img/cover_busqueda.png)">
		<div class="container">
			<h2 id="title-escaparate">Categoría: <span style="color:#3e6d38"><?php echo $title?></span></h2>
		</div>
	</section>
					...
					<article>
						<div class="row">
							<div class="col-12">
								...
								<form action="<?php echo $urlserver.'categorias'?>" accept-charset="UTF-8" method="get">
									<div class="row">
							...
							<div class="col-12 col-sm-6 col-md-4 col-lg-3">
								<a href="<?php echo SITEUSER.$categoria_tienda.'/'.$nick_tienda.'/'.$url_producto?>" class="link-product">
								<img src="<?php echo SITEUSER.'img/tiendas/'.$nick_tienda.'/'.$img_a_producto?>" class="img-fluid d-block w-100" alt="">
							...
							<div class="col-12">
								<blockquote class="blockquote text-center">
									...
									<h6>No se han encontrado productos.</h6>
						...
						<div class="clearfix"></div>
						<?php
							if(($row_cnt!=0) AND ($total_pages!=1)){
							$link =$urlserver.'categorias?s='.$busqueda.'&ordenar='.$ordenar.'&cat='.$cat.'&';
						?>
